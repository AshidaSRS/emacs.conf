;;; custom-nord-theme.el --- An arctic, north-bluish clean and elegant theme
...
(deftheme custom-nord "An arctic, north-bluish clean and elegant theme")

(defgroup custom-nord nil
  "Nord theme customizations.
  The theme has to be reloaded after changing anything in this group."
  :group 'faces)

(defcustom nord-comment-brightness 5
...
(defun nord-theme--brightened-comment-color (percent)
  ...
  The comment color brightness has been increased by 10% by default.
  Please see https://github.com/arcticicestudio/nord-emacs/issues/73 for more details."
  (nth 5 nord-theme--brightened-comments))
...
(let ((class '((class color) (min-colors 89)))
      (nord-custom (if (nord-display-truecolor-or-graphic-p) "#1A1E25" "#1A1E25"))
      ...
      (nord-annotation-custom (if (nord-display-truecolor-or-graphic-p) "#70a8f2" "#70a8f2"))
      ...
      (nord-string (if (nord-display-truecolor-or-graphic-p) "#1478D8" "#1478D8"))
...
;;;; +------------+
  (custom-theme-set-faces
   ...
   `(default ((,class (:foreground ,nord4 :background ,nord-custom))))
   ...
   `(font-lock-regexp-grouping-backslash ((,class (:foreground ,nord-operator))))
   `(font-lock-regexp-grouping-construct ((,class (:foreground ,nord-operator))))
   `(font-lock-string-face ((,class (:foreground ,nord-annotation-custom))))
   ...
   `(custom-themed ((,class (:foreground ,nord2 :background ,nord2))))
